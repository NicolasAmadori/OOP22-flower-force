...
public final class Level {
    ...
    private static final List<Function<Point2D, Zombie>> AVAILABLE_ZOMBIES = List.of(
            (pos) -> new ZombieFactory().basic(pos),
            (pos) -> new ZombieFactory().conehead(pos),
            (pos) -> new ZombieFactory().runner(pos),
            (pos) -> new ZombieFactory().newspaper(pos),
            (pos) -> new ZombieFactory().buckethead(pos),
            (pos) -> new ZombieFactory().quarterback(pos)
    );
    ...
    private static final Function<Point2D, Zombie> ZOMBIE_BOSS = (pos) -> new ZombieFactory().gargantuar(pos);
